...
func _process(delta):
	if Input.is_action_just_released("right%s"%id) and cursor_location=="player":
		changed=true
		...
		if cursor_position[0]!=2:
			...
			cursor_position[0]=0
	elif Input.is_action_just_released("left%s"%id) and cursor_location=="player":
		changed=true
		cursor_last_position[0]=cursor_position[0]
		cursor_last_position[1]=cursor_position[1]
		if cursor_position[0]!=0:
			...
			cursor_position[0]=2
	elif Input.is_action_just_released("up%s"%id) and cursor_location=="player":
		changed=true
		cursor_last_position[0]=cursor_position[0]
		cursor_last_position[1]=cursor_position[1]
		if cursor_position[1]!=0:
			...
			cursor_position[1]=2
	elif Input.is_action_just_released("down%s"%id) and cursor_location=="player":
		changed=true
		cursor_last_position[0]=cursor_position[0]
		cursor_last_position[1]=cursor_position[1]
	...
	if Input.is_action_just_released("buy%s"%id):
		...
		$TileMap.set_cell(cursor_position[0],cursor_position[1],0)
